...
class FMP4 {
     ...
     * @returns {Uint8Array}
     */
    static extension(version: number, flag: number) {
    ...
    static trun(data: IObject, sdtpLength: number) {
        let id = data.id;
        let ceil = id === 1 ? 16 : 12;
        let buffer = new Buffer();
        ...
        buffer.write(
            FMP4.size(20 + ceil * data.samples.length),
            FMP4.type('trun'),
            FMP4.extension(0, data.flags),
            sampleCount,
            offset
        );
        data.samples.forEach((item: IObject) => {
    ...
    static traf(data: IObject) {
        let buffer: Buffer = new Buffer();
        let size: number = 8;
        let tfhd: Uint8Array = FMP4.tfhd(data.id);
        let tfdt: Uint8Array = FMP4.tfdt(data.time);
        let sdtp: Uint8Array = FMP4.sdtp(data);
        let trun: Uint8Array = FMP4.trun(data, sdtp.byteLength);
    ...
    static moof(data: IObject) {
        let buffer: Buffer = new Buffer();
        let size: number = 8;
        let mfhd: Uint8Array = FMP4.mfhd();
        let traf: Uint8Array = FMP4.traf(data);
    ...
    static mvhd(duration: number, timescale: number): Uint8Array {
        let buffer = new Buffer();
        duration *= timescale;
    ...
    static tkhd(data: IObject): Uint8Array {
        let buffer = new Buffer();
        let id = data.id;
    ...
    static mdhd(timescale: number, duration: number = 0) {
        let buffer = new Buffer();
        duration *= timescale;
        const upperWordDuration = Math.floor(duration / (UINT32_MAX + 1));
        const lowerWordDuration = Math.floor(duration % (UINT32_MAX + 1));
    ...
    static esds(config: number[] = [43, 146, 8, 0]) {
    ...
    static mp4a(data: IObject) {
        ...
        let esds: Uint8Array = FMP4.esds(data.audioConfig as number[]);
        buffer.write(
            FMP4.size(8 + content.byteLength + esds.byteLength),
            FMP4.type('mp4a'),
            content,
            esds
        );
    ...
    static mdia(data: IObject): Uint8Array {
        let buffer: Buffer = new Buffer();
        let size: number = 8;
    ...
    static videoTrak(data: IObject) {
        ...
        [tkhd, mdia].forEach((item: Uint8Array) => {
            size += item.byteLength
        });
    ...
    static mvex(duration: number, timeScale: number): Uint8Array {
        let buffer = new Buffer();
    ...
    static moov(data: IObject): Uint8Array {
        let buffer = new Buffer;
        let size: number = 8;
        let videoOnly:boolean = data.videoOnly;
        let _timeScale = data.timeScale || 90000;
        let mvhd: Uint8Array = FMP4.mvhd(data.duration, _timeScale);
        let trak1: Uint8Array = FMP4.videoTrak(data);
        let trak2: Uint8Array = FMP4.audioTrak(data);
        let mvex: Uint8Array = FMP4.mvex(data.duration, _timeScale);
        let tempBoxList = videoOnly ? [mvhd, trak1, mvex] : [mvhd, trak1, trak2, mvex];
